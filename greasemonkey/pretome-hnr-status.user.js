...
var C_HISTORY_RATIO = 3;
var C_HISTORY_SEED = 7;
var C_HISTORY_HNR = 8;
var C_HISTORY_COUNT = 9;
...
function hnr_rainbow() {
    ...
    for (i = 1; i < table.rows.length; i += 1) {
        ...
        if (row.cells.length === C_HISTORY_COUNT) {
            ...
            s = row.cells[C_HISTORY_SEED];
            h = row.cells[C_HISTORY_HNR];
    ...
    for (i = 1; i < table.rows.length; i += 1) {
        ...
        if (row.cells.length === C_PEER_COUNT) {
            ...
            s = row.cells[C_PEER_SEED];
            h = row.cells[C_PEER_HNR];
...
if (/\/usertorrents\.php\?id=[0-9]+/.exec(document.location.href) !== null) {
    hnr_fixheader("Download History for ");
    hnr_fixheader("Peer Details for ");
    hnr_rainbow();
}
